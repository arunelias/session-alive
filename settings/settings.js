...
function setItem() {
    ...
    selectedRuleId = null;
    const refreshSettings = chrome.storage.local.get();
...
document.getElementById('rulesList').addEventListener('click', function (e) {
    ...
    if (ruleItem) {
        ...
        const gettingSettingsByKey = chrome.storage.local.get(selectedRuleId);
...
document.getElementById('deleteButton').addEventListener('click', function () {
    ...
    if (confirm("Are you sure you want to delete this rule?")) {
        return chrome.storage.local.remove(aliveRuleId).then(removedItem, onError);
    }
});
...
/**
 * Stores the current rule settings in chrome.storage.local.
...
function storeSettings() {
    ...
    return chrome.storage.local.set(rules).then(setItem, onError);
}
/**
...
/**
 * Saves imported rules to chrome.storage.local.
...
function saveRules(aliveData) {
    return chrome.storage.local.set(aliveData).then(setItem, onError);
}
