/**
 * @fileoverview Background script for Session Alive extension.
...
const initializationPromises = [
  browser.storage.local.get().then(updateRules, onError),
  browser.storage.session.get().then(updateVariables, onError)
];
...
function getTranslatedNotification({ title, message, key }) {
  ...
  const translatedTitle = browser.i18n.getMessage(`${key}Title`) || title;
  const translatedMessage = browser.i18n.getMessage(`${key}Message`) || message;
...
function updateBadge() {
  ...
  if (totRunningRules > 0) { browser.action.setBadgeText({ text: totRunningRules.toString() }); }
  else { browser.action.setBadgeText({ text: "" }); }
}

/**
 * Updates the aliveRules object with rules from local storage.
 * @param {Object} aliveSettings - Session alive settings from browser.storage.local.
 * @returns {void}
 */
 ...
 * @param {Object} aliveSession - Running rules from browser.storage.session.
 * @returns {void}
 */
...
/**
 * Adds a listener for browser.runtime.onMessage to handle messages from content or popup scripts.
